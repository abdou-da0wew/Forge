...
// CheckBreakers evaluates all circuit breakers and returns the first triggered one
func (w *Watchdog) checkBreakers(stats *container.ContainerStats) *BreakerType {
        // Check CPU sustained
        if w.checkCPUSustained(stats) {
                return w.trigger(BreakerCPUSustained, "CPU > %d%% for %d seconds", 
                        w.cfg.CPUSustainedThresholdPct, w.cfg.CPUSustainedDurationSeconds)
        }

        // Check RAM pressure
        if w.checkRAMPressure(stats) {
                return w.trigger(BreakerRAMPressure, "RAM > %.1f%% of limit", 
                        float64(w.cfg.RAMThresholdPct))
        }

        // Check PID flood
        if w.checkPIDFlood(stats) {
                return w.trigger(BreakerPIDFlood, "PIDs > %d", w.cfg.PIDLimit)
        }

        // Check disk storm
        if w.checkDiskStorm(stats) {
                return w.trigger(BreakerDiskStorm, "Disk writes > %d MB in session", 
                        w.cfg.DiskWriteLimitMB)
        }

        // Check network scan
        if w.checkNetworkScan(stats) {
                return w.trigger(BreakerNetworkScan, "Network scan detected")
        }
...
func (w *Watchdog) trigger(breaker BreakerType, format string, args ...interface{}) *BreakerType {
